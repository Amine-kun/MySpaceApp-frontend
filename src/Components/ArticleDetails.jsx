...
import MSL from './MSL';
import { pinDetailMorePinQuery, pinDetailQuery, articleDetailQuery, articleDetailMorePinQuery } from '../UtilityFunctions/data';
...
const ArticleDetails = ({ user }) => {
  ...
  const fetchPinDetails = () => {
    const query = articleDetailQuery(articleId);

    if (query) {
      client.fetch(`${query}`).then((data) => {
        setPinDetail(data[0]);
        if (data[0]) {
          const query1 = articleDetailMorePinQuery(data[0]);
    ...
    <>
      {pinDetail && (
        <div className="flex xl:flex-row flex-col m-auto bg-white" style={{ maxWidth: '1500px', borderRadius: '32px' }}>
          <div className="flex justify-center items-center md:items-start flex-initial lg:mx-2 lg:my-2">
            <img
          ...
          <div className="w-full p-5 flex-1 xl:min-w-620">
            ...
            <div>
              ...
              <p className="mt-3 text-gray-600 font-semibold">{pinDetail.about}</p>
            </div>
